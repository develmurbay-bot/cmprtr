...
export default function GalleryManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const imageUrl = uploadedImage || formData.image_url;
  ...
  useEffect(() => {
    if (formData.title && formData.category && !editingItem && !uploadedImage) {
      const newImageUrl = generatePlaceholderUrl(formData.title, formData.category);
      setFormData(prev => ({ ...prev, image_url: newImageUrl }));
    }
  }, [formData.title, formData.category, editingItem, uploadedImage]);
