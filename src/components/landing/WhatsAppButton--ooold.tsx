...
import { getDefaultSettings, generateWhatsAppUrl, type AppSettings } from '@/lib/settings';
...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        console.warn('Failed to fetch settings for WhatsApp button:', error);
      }
    };
  ...
  const handleWhatsAppClick = () => {
    const whatsappUrl = generateWhatsAppUrl(settings);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        ...
        title={`Chat via WhatsApp: ${settings.whatsapp_number}`}
      >
